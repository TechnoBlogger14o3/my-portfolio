...
const FloatingText = ({ position, text, color = '#ffffff' }: {
  position: [number, number, number];
  text: string;
  color?: string;
}) => {
  ...
  useFrame((state) => {
    if (textRef.current) {
      textRef.current.position.y = position[1] + Math.sin(state.clock.elapsedTime * 0.6) * 0.3;
    }
  });
...
const Scene = () => {
  ...
  const texts = [
    { position: [0, 4, 0] as [number, number, number], text: 'Aman Shekhar' },
    { position: [-5, 0, 0] as [number, number, number], text: 'Mobile Dev' },
    { position: [5, 0, 0] as [number, number, number], text: 'Tech Enthusiast' },
    { position: [0, -4, 0] as [number, number, number], text: 'Chess Player' },
  ];
